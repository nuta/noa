...
fn send_requests(lsp: &crate::language::Lsp, rx: Receiver<Request>, mut stdin: ChildStdin) {
...
impl Lsp {
    ...
    pub fn send(&mut self, lang: &'static Language, req: Request) {
        if let Some(lsp) = &lang.lsp {
            let server = match self.servers.get(lang) {
                Some(server) => server,
                None => {
                    // The server has not yet been spawned.
