...
pub struct Editor {
    ...
    terminal: Terminal,
    finder: Finder,
    finder_input: LineEdit,
    ned_changes: Option<ned::Changes>,
    ned_input: LineEdit,
    ned_preview_rope: Rope,
...
impl Editor {
    pub fn new() -> Editor {
        ...
        Editor {
            ...
            finder_input: LineEdit::new(),
            ned_changes: None,
            ned_input: LineEdit::new(),
            ned_preview_rope: Rope::new(),
            event_queue: rx,
            buffers: HashMap::new(),
    ...
    pub fn run(&mut self) -> ExitStatus {
        loop {
            ...
            match self.mode {
                ...
                EditorMode::Finder => {
                    self.terminal
                        .draw_finder(&self.finder, &mut self.finder_input);
                }
                EditorMode::Ned => {
                    let buffer = self.current_buffer.borrow();
                    self.terminal.draw_ned(
                        &*buffer,
                        &self.ned_preview_rope,
                        self.ned_changes.as_ref(),
                        &mut self.ned_input,
                        self.notification.borrow().as_ref(),
                    );
                }
    ...
    fn warn<T: Into<String>>(&self, message: T) {
    ...
    fn execute_ned(&mut self, preview: bool) {
        let mut buffer = self.current_buffer.borrow_mut();
        let current = buffer.cursor_as_range();
        let (limit, rope) = if preview {
            self.ned_preview_rope = buffer.rope_mut().clone();
            (Some(100), &mut self.ned_preview_rope)
        } else {
            (None, buffer.rope_mut())
        };

        match ned::Engine::new(&self.ned_input.text()) {
            Ok(mut engine) => match engine.execute(rope, &current, limit) {
                Ok(changes) => {
                    self.ned_changes = Some(changes);
    ...
    fn update_modes(&mut self) {
        match self.mode {
            EditorMode::Normal => {}
            EditorMode::Ned => {
                self.execute_ned(true);
            }
            EditorMode::Finder => {
                let query = self.finder_input.text();
    ...
    fn handle_key_event(&mut self, key: KeyEvent) {
        ...
        match self.mode {
            EditorMode::Normal => match (key.code, key.modifiers) {
                ...
                (KeyCode::Char('w'), CTRL) => {
                    self.ned_input.clear();
                    self.ned_changes = None;
                    self.mode = EditorMode::Ned;
                }
                //
                //  Text Editing.
                //
            ...
            EditorMode::Finder => match (key.code, key.modifiers) {
                ...
                (KeyCode::Char(ch), NONE) | (KeyCode::Char(ch), SHIFT) => {
                    self.finder_input.insert_char(ch);
                }
                (KeyCode::Enter, NONE) => {
                ...
                (KeyCode::Backspace, NONE) => {
                    self.finder_input.backspace();
                }
                (KeyCode::Delete, NONE) | (KeyCode::Char('d'), CTRL) => {
                    self.finder_input.delete();
                }
                (KeyCode::Left, NONE) => {
                    self.finder_input.move_left();
                }
                (KeyCode::Right, NONE) => {
                    self.finder_input.move_right();
                }
                (KeyCode::Up, _) => {
                ...
                (KeyCode::Char('a'), CTRL) => {
                    self.finder_input.move_to_beginning_of_line();
                }
                (KeyCode::Char('e'), CTRL) => {
                    self.finder_input.move_to_end_of_line();
                }
                (KeyCode::Char('b'), ALT) => {
                    self.finder_input.move_to_prev_word();
                }
                (KeyCode::Char('f'), ALT) => {
                    self.finder_input.move_to_next_word();
                }
                _ => {}
            },
            EditorMode::Ned => match (key.code, key.modifiers) {
                (KeyCode::Char('q'), CTRL) | (KeyCode::Esc, _) => {
                    self.mode = EditorMode::Normal;
                }
                (KeyCode::Char(ch), NONE) | (KeyCode::Char(ch), SHIFT) => {
                    self.ned_input.insert_char(ch);
                }
                (KeyCode::Enter, NONE) => {
                    self.execute_ned(false);
                    self.mode = EditorMode::Normal;
                }
                (KeyCode::Backspace, NONE) => {
                    self.ned_input.backspace();
                }
                (KeyCode::Delete, NONE) | (KeyCode::Char('d'), CTRL) => {
                    self.ned_input.delete();
                }
                (KeyCode::Left, NONE) => {
                    self.ned_input.move_left();
                }
                (KeyCode::Right, NONE) => {
                    self.ned_input.move_right();
                }
                (KeyCode::Char('a'), CTRL) => {
                    self.ned_input.move_to_beginning_of_line();
                }
                (KeyCode::Char('e'), CTRL) => {
                    self.ned_input.move_to_end_of_line();
                }
                (KeyCode::Char('b'), ALT) => {
                    self.ned_input.move_to_prev_word();
                }
                (KeyCode::Char('f'), ALT) => {
                    self.ned_input.move_to_next_word();
                }
                _ => {}
            },
        }
    ...
    fn handle_key_batch_event(&mut self, s: String) {
        match self.mode {
            ...
            EditorMode::Finder => {
                self.finder_input.insert(&s);
            }
            EditorMode::Ned => {
                self.ned_input.insert(&s);
            }
        }
    }
